...
TBmodel <- function(disease_progression, pop, mortality, start, steps, ari, tx.cure.rate, stable=T, start.trt, protect=0){
  ...
  for(i in start:steps){
    ...
    if(i>1){
      ...
      sim.pop$old.reinf.inf <- sim.pop$old.reinf.inf*(1-dis.prob[6])*(1-ari)  + sim.pop$inf.reinf.4yr*(1-dis.prob[5])*(1-ari)
      sim.pop$inf.reinf.4yr <- sim.pop$inf.reinf.3yr*(1-dis.prob[4])*(1-ari)
      sim.pop$inf.reinf.3yr <- sim.pop$inf.reinf.2yr*(1-dis.prob[3])*(1-ari)
      sim.pop$inf.reinf.2yr <- sim.pop$inf.reinf.1yr*(1-dis.prob[2])*(1-ari)
      sim.pop$inf.reinf.1yr <- sim.pop$new.reinf.inf.previous*(1-dis.prob[1])*(1-ari) 
      ...
      sim.pop$progress2tbreinf <- sim.pop$new.reinf.inf*dis.prob[1]*(1-protect) + sim.pop$inf.reinf.1yr*dis.prob[2] + sim.pop$inf.reinf.2yr*dis.prob[3] + sim.pop$inf.reinf.3yr*dis.prob[4] + sim.pop$inf.reinf.4yr*dis.prob[5]  + sim.pop$old.reinf.inf*dis.prob[6]
