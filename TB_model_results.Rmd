...
setwd("/projectnb/cbs/samalate/TBReModel")
...
min_prev <- df2  %>% filter(diff>0) %>% group_by(prev_target, protect, tot.dis.prog) %>% slice(which.min(diff))
min_prev=min_prev[-1,] %>% filter(prev_target>0)

min_prev %>% group_by(protect, tot.dis.prog, tb.dis) %>% summarise(m=mean(time))

ggplot(data=min_prev) + geom_point(aes(x=ari, y=time)) + facet_grid(protect~tot.dis.prog) 


ggplot(data=min_prev) + geom_point(aes(x=ari, y=tb.dis)) + facet_grid(protect~tot.dis.prog) + geom_hline(aes(yintercept =10000)) + geom_line(aes(x=ari_par, y=prev_target*10,  color="Target"))


time = min_prev %>% select(protect, tot.dis.prog, time, ari_par, tb.dis)
...
```{r}
all_res=readRDS("TB_model_2025-09-15.rds")
max=49
...
ggplot(data=t) + geom_point(aes(x=ari, y=tb.dis)) + facet_grid(protect~tot.dis.prog) + geom_hline(aes(yintercept = 10000))
